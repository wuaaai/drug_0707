...
from preprocess.OverWrite_data import Patient, Event
from preprocess.OverWrite_sample_dataset import SampleEHRDataset
...
class BaseEHRDataset(ABC):
    ...
    每个特定的数据集都是这个抽象类的一个子类，然后可以通过调用“self.set_task（）”转换为不同任务的样本数据集。
    ...
    # 加载映射工具CrossMap,即一种文件code转换成另一种code
    ...
    # 抽象方法，需要在子类中实现。这个方法应该调用不同的解析函数来处理各个表，并将结果合并到患者对象中。
    ...
    def parse_tables(self) -> Dict[str, Patient]:
        ...
        tic = time.time()
        ### 加了个进度条
        # patients = tqdm(self.parse_basic_info(patients))
        ...
        for table in tqdm(self.tables, desc="Parsing tables"):
            try:
